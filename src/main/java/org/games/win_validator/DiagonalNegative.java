...
public class DiagonalNegative implements WinCheck {
    @Override
    public Win win(Settings settings) {
        ...
        while ((row < range) && (win.win() == WinType.NOT_WIN)) {
            column = row + settings.getRequireToWin() - 1;
    ...
    public Win check(Settings settings, int initSquare) {
        ...
        return  count >= settings.getRequireToWin() ?
                new Win(WinType.DIAGONAL_NEGATIVE, initSquare) : new Win(WinType.NOT_WIN,0);
    }
}
